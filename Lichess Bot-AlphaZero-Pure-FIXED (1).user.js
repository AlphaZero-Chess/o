...
function handlePositionMessage(message) {
    ...
    if (isOurTurn) {
        // ═══════════════════════════════════════════════════════════════
        // SPECIAL FLOW: Position-version-based calculation control
        // ═══════════════════════════════════════════════════════════════
        
        // RULE 1: White at v=0 (starting position) - BLOCK auto-play
        if (currentWsV === 0 && myColor === 'w') {
            debugLog("[MANUAL]", "⏸️ WHITE MOVE #1: Manual intervention required");
            debugLog("[MANUAL]", "  Bot will NOT auto-play until manual move is made");
            opponentMoveConfirmed = false; // Block calculation
            return; // Wait for manual intervention
        }
        
        // RULE 2: v=1 (Black's move after White manual) - FORCE auto-play
        if (currentWsV === 1) {
            debugLog("[AUTO]", "🤖 BLACK MOVE #2: Auto-play starting (after manual White move #1)");
            
            // Clear any stale human-move flags
            humanMovedRecently = false;
            ...
            // Confirm opponent move and schedule calculation
            ...
            messageDebounceTimer = setTimeout(() => {
                debugLog("[AUTO]", "✅ Executing Black move #2 calculation");
                scheduleCalculate();
        ...
        // RULE 3: White at v=2 (White's second move) - FORCE auto-play
        if (currentWsV === 2 && myColor === 'w') {
            debugLog("[AUTO]", "🤖 WHITE MOVE #3: Auto-play enabled (after Black move #2)");
            ...
            messageDebounceTimer = setTimeout(() => {
                debugLog("[AUTO]", "✅ Executing White move #3 calculation");
                scheduleCalculate();
            }, CONFIG.messageDebounce);
        ...
        // ═══════════════════════════════════════════════════════════════
        // RULE 4: Normal flow for all moves v≥3 (timing-based detection)
        // ═══════════════════════════════════════════════════════════════
